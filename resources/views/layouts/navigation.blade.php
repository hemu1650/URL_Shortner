<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                    ...
                    @php
                        ...
                        if ($user) {
                            if ($user->role === 'superadmin') {
                                $dashboardRoute = url('/superadmin/dashboard');
                            } elseif ($user->role === 'admin') {
                                $dashboardRoute = url('/admin/dashboard');
                            } elseif ($user->role === 'member') {
                                $dashboardRoute = url('/urls');
                            }
                        }
                    @endphp
                ...
                <!-- Navigation Links -->
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex" style="display: none">
                    ...
                    @if (Auth::user()->role === 'superadmin')
                        <x-nav-link :href="route('superadmin.invite')" :active="request()->routeIs('superadmin.invite')">
                            {{ __('Invite Admin') }}
                        </x-nav-link>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1" style="display: none">
            ...
            @if (Auth::user()->role === 'superadmin')
                <x-responsive-nav-link :href="route('superadmin.invite')" :active="request()->routeIs('superadmin.invite')">
                    {{ __('Invite Admin') }}
                </x-responsive-nav-link>
            @elseif (Auth::user()->role === 'admin')
                <x-responsive-nav-link :href="route('admin.invite')" :active="request()->routeIs('admin.invite')">
                    {{ __('Invite Member') }}
                </x-responsive-nav-link>
            @else
